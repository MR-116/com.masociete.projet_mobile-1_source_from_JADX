...
class TypefaceCompatBaseImpl implements TypefaceCompatImpl {
    ...
    class C02741 implements StyleExtractor<FontInfo> {
        C02741() {
        }
    ...
    class C02752 implements StyleExtractor<FontFileResourceEntry> {
        C02752() {
        }
    ...
    private static <T> T findBestFont(T[] fonts, int style, StyleExtractor<T> extractor) {
        ...
        int targetWeight = (style & 1) == 0 ? C1084x.f3269t : C1084x.f3262h;
        if ((style & 2) != 0) {
            isTargetItalic = true;
        } else {
    ...
    protected FontInfo findBestInfo(FontInfo[] fonts, int style) {
        return (FontInfo) findBestFont(fonts, style, new C02741());
    }
    ...
    private FontFileResourceEntry findBestEntry(FontFamilyFilesResourceEntry entry, int style) {
        return (FontFileResourceEntry) findBestFont(entry.getEntries(), style, new C02752());
    }
