...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0222R.styleable.FontFamily);
        String authority = array.getString(C0222R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C0222R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C0222R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C0222R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C0222R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C0222R.styleable.FontFamily_fontProviderFetchTimeout, 500);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0222R.styleable.FontFamilyFont);
        int weight = array.getInt(C0222R.styleable.FontFamilyFont_fontWeight, 400);
        if (1 != array.getInt(C0222R.styleable.FontFamilyFont_fontStyle, 0)) {
            isItalic = false;
        }
        int resourceId = array.getResourceId(C0222R.styleable.FontFamilyFont_font, 0);
        String filename = array.getString(C0222R.styleable.FontFamilyFont_font);
        array.recycle();
        while (parser.next() != 3) {
            skip(parser);
        }
