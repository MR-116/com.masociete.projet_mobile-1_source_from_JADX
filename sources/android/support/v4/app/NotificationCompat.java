...
import android.support.compat.C0222R;
...
public class NotificationCompat {
    ...
    public static class Action extends android.support.v4.app.NotificationCompatBase.Action {
        ...
        public static final Factory FACTORY = new C02641();
        ...
        static class C02641 implements Factory {
            C02641() {
            }
    ...
    public static abstract class Style {
        ...
        public RemoteViews applyStandardTemplate(boolean showSmallIcon, int resId, boolean fitIn1U) {
            ...
            if (VERSION.SDK_INT >= 16 && VERSION.SDK_INT < 21) {
                if (minPriority) {
                    contentView.setInt(C0222R.id.notification_background, "setBackgroundResource", C0222R.drawable.notification_bg_low);
                    contentView.setInt(C0222R.id.icon, "setBackgroundResource", C0222R.drawable.notification_template_icon_low_bg);
                } else {
                    contentView.setInt(C0222R.id.notification_background, "setBackgroundResource", C0222R.drawable.notification_bg);
                    contentView.setInt(C0222R.id.icon, "setBackgroundResource", C0222R.drawable.notification_template_icon_bg);
                }
            }
            if (this.mBuilder.mLargeIcon != null) {
                if (VERSION.SDK_INT >= 16) {
                    contentView.setViewVisibility(C0222R.id.icon, 0);
                    contentView.setImageViewBitmap(C0222R.id.icon, this.mBuilder.mLargeIcon);
                } else {
                    contentView.setViewVisibility(C0222R.id.icon, 8);
                }
                if (showSmallIcon && this.mBuilder.mNotification.icon != 0) {
                    int backgroundSize = res.getDimensionPixelSize(C0222R.dimen.notification_right_icon_size);
                    int iconSize = backgroundSize - (res.getDimensionPixelSize(C0222R.dimen.notification_small_icon_background_padding) * 2);
                    if (VERSION.SDK_INT >= 21) {
                        contentView.setImageViewBitmap(C0222R.id.right_icon, createIconWithBackground(this.mBuilder.mNotification.icon, backgroundSize, iconSize, this.mBuilder.getColor()));
                    } else {
                        contentView.setImageViewBitmap(C0222R.id.right_icon, createColoredBitmap(this.mBuilder.mNotification.icon, -1));
                    }
                    contentView.setViewVisibility(C0222R.id.right_icon, 0);
                }
            } else if (showSmallIcon && this.mBuilder.mNotification.icon != 0) {
                contentView.setViewVisibility(C0222R.id.icon, 0);
                if (VERSION.SDK_INT >= 21) {
                    contentView.setImageViewBitmap(C0222R.id.icon, createIconWithBackground(this.mBuilder.mNotification.icon, res.getDimensionPixelSize(C0222R.dimen.notification_large_icon_width) - res.getDimensionPixelSize(C0222R.dimen.notification_big_circle_margin), res.getDimensionPixelSize(C0222R.dimen.notification_small_icon_size_as_large), this.mBuilder.getColor()));
                } else {
                    contentView.setImageViewBitmap(C0222R.id.icon, createColoredBitmap(this.mBuilder.mNotification.icon, -1));
                }
            }
            if (this.mBuilder.mContentTitle != null) {
                contentView.setTextViewText(C0222R.id.title, this.mBuilder.mContentTitle);
            }
            if (this.mBuilder.mContentText != null) {
                contentView.setTextViewText(C0222R.id.text, this.mBuilder.mContentText);
                showLine3 = true;
            }
            boolean hasRightSide = VERSION.SDK_INT < 21 && this.mBuilder.mLargeIcon != null;
            if (this.mBuilder.mContentInfo != null) {
                contentView.setTextViewText(C0222R.id.info, this.mBuilder.mContentInfo);
                contentView.setViewVisibility(C0222R.id.info, 0);
                showLine3 = true;
                hasRightSide = true;
            } else if (this.mBuilder.mNumber > 0) {
                if (this.mBuilder.mNumber > res.getInteger(C0222R.integer.status_bar_notification_info_maxnum)) {
                    contentView.setTextViewText(C0222R.id.info, res.getString(C0222R.string.status_bar_notification_info_overflow));
                } else {
                    contentView.setTextViewText(C0222R.id.info, NumberFormat.getIntegerInstance().format((long) this.mBuilder.mNumber));
                }
                contentView.setViewVisibility(C0222R.id.info, 0);
                showLine3 = true;
                hasRightSide = true;
            } else {
                contentView.setViewVisibility(C0222R.id.info, 8);
            }
            if (this.mBuilder.mSubText != null && VERSION.SDK_INT >= 16) {
                contentView.setTextViewText(C0222R.id.text, this.mBuilder.mSubText);
                if (this.mBuilder.mContentText != null) {
                    contentView.setTextViewText(C0222R.id.text2, this.mBuilder.mContentText);
                    contentView.setViewVisibility(C0222R.id.text2, 0);
                    showLine2 = true;
                } else {
                    contentView.setViewVisibility(C0222R.id.text2, 8);
                }
            }
            if (showLine2 && VERSION.SDK_INT >= 16) {
                if (fitIn1U) {
                    contentView.setTextViewTextSize(C0222R.id.text, 0, (float) res.getDimensionPixelSize(C0222R.dimen.notification_subtext_size));
                }
                contentView.setViewPadding(C0222R.id.line1, 0, 0, 0, 0);
            }
            if (this.mBuilder.getWhenIfShowing() != 0) {
                if (!this.mBuilder.mUseChronometer || VERSION.SDK_INT < 16) {
                    contentView.setViewVisibility(C0222R.id.time, 0);
                    contentView.setLong(C0222R.id.time, "setTime", this.mBuilder.getWhenIfShowing());
                } else {
                    contentView.setViewVisibility(C0222R.id.chronometer, 0);
                    contentView.setLong(C0222R.id.chronometer, "setBase", this.mBuilder.getWhenIfShowing() + (SystemClock.elapsedRealtime() - System.currentTimeMillis()));
                    contentView.setBoolean(C0222R.id.chronometer, "setStarted", true);
                }
                hasRightSide = true;
            }
            contentView.setViewVisibility(C0222R.id.right_side, hasRightSide ? 0 : 8);
            contentView.setViewVisibility(C0222R.id.line3, showLine3 ? 0 : 8);
            return contentView;
        }
        ...
        private Bitmap createIconWithBackground(int iconId, int size, int iconSize, int color) {
            int i = C0222R.drawable.notification_icon_background;
            if (color == 0) {
                color = 0;
            }
        ...
        public void buildIntoRemoteViews(RemoteViews outerView, RemoteViews innerView) {
            ...
            outerView.removeAllViews(C0222R.id.notification_main_column);
            outerView.addView(C0222R.id.notification_main_column, innerView.clone());
            outerView.setViewVisibility(C0222R.id.notification_main_column, 0);
            if (VERSION.SDK_INT >= 21) {
                outerView.setViewPadding(C0222R.id.notification_main_column_container, 0, calculateTopPadding(), 0, 0);
            }
        }

        private void hideNormalContent(RemoteViews outerView) {
            outerView.setViewVisibility(C0222R.id.title, 8);
            outerView.setViewVisibility(C0222R.id.text2, 8);
            outerView.setViewVisibility(C0222R.id.text, 8);
        }

        private int calculateTopPadding() {
            Resources resources = this.mBuilder.mContext.getResources();
            float largeFactor = (constrain(resources.getConfiguration().fontScale, 1.0f, 1.3f) - 1.0f) / 0.29999995f;
            return Math.round(((1.0f - largeFactor) * ((float) resources.getDimensionPixelSize(C0222R.dimen.notification_top_pad))) + (((float) resources.getDimensionPixelSize(C0222R.dimen.notification_top_pad_large_text)) * largeFactor));
        }
    ...
    public static final class CarExtender implements Extender {
        ...
        public static class UnreadConversation extends android.support.v4.app.NotificationCompatBase.UnreadConversation {
            static final android.support.v4.app.NotificationCompatBase.UnreadConversation.Factory FACTORY = new C02651();
            ...
            static class C02651 implements android.support.v4.app.NotificationCompatBase.UnreadConversation.Factory {
                C02651() {
                }
    ...
    public static class DecoratedCustomViewStyle extends Style {
        ...
        private RemoteViews createRemoteViews(RemoteViews innerView, boolean showActions) {
            ...
            RemoteViews remoteViews = applyStandardTemplate(true, C0222R.layout.notification_template_custom_big, false);
            remoteViews.removeAllViews(C0222R.id.actions);
            ...
            if (showActions && this.mBuilder.mActions != null) {
                ...
                if (numActions > 0) {
                    ...
                    for (int i = 0; i < numActions; i++) {
                        remoteViews.addView(C0222R.id.actions, generateActionButton((Action) this.mBuilder.mActions.get(i)));
                    }
                }
            }
            if (!actionsVisible) {
            ...
            remoteViews.setViewVisibility(C0222R.id.actions, actionVisibility);
            remoteViews.setViewVisibility(C0222R.id.action_divider, actionVisibility);
        ...
        private RemoteViews generateActionButton(Action action) {
            ...
            RemoteViews button = new RemoteViews(this.mBuilder.mContext.getPackageName(), tombstone ? C0222R.layout.notification_action_tombstone : C0222R.layout.notification_action);
            button.setImageViewBitmap(C0222R.id.action_image, createColoredBitmap(action.getIcon(), this.mBuilder.mContext.getResources().getColor(C0222R.color.notification_action_color_filter)));
            button.setTextViewText(C0222R.id.action_text, action.title);
            if (!tombstone) {
                button.setOnClickPendingIntent(C0222R.id.action_container, action.actionIntent);
            }
            if (VERSION.SDK_INT >= 15) {
                button.setContentDescription(C0222R.id.action_container, action.title);
            }
            return button;
        }
    }
